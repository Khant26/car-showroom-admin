...
const ImageUpload = ({
  ...
  uploadMode = "api", // "api" or "file" - determines if files are uploaded immediately or passed as File objects
}) => {
  ...
  const handleFileSelect = async (files) => {
    ...
    if (uploadMode === "file") {
      // Pass files directly without uploading
      const newFiles = filesToUpload.map((file) => file);
    ...
    try {
      ...
      if (error.response?.status === 404) {
        errorMessage =
          "Upload service not available. Please ensure the backend server is running.";
                  ...
                  src={
                    typeof image === "string"
                      ? image.startsWith("/")
                        ...
                        : image
                      : URL.createObjectURL(image) // For File objects
                  }
                  alt={`Preview ${index + 1}`}
